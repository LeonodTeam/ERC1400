...
const INVALID_CERTIFICATE = '0x0b8abf945f8e4f95c0ee2bd241d7d3403edcc41761ee833a817dadf90a6d28d9a4d1eaa445b873976a685d6254230e29c1c75266a096194a3b078bd02a826ac6d2';
const INVALID_CERTIFICATE_SENDER = '0x0b8abf945f8e4f95c0ee2bd241d7d3403edcc41761ee833a817dadf90a6d28d9a4d1eaa445b873976a685d6254230e29c1c75266a096194a3b078bd02a826ac6d2';
const INVALID_CERTIFICATE_RECIPIENT = '0x0b8abf945f8e4f95c0ee2bd241d7d3403edcc41761ee833a817dadf90a6d28d9a4d1eaa445b873976a685d6254230e29c1c75266a096194a3b078bd02a826ac6d2';
...
contract('ERC1400', function ([owner, operator, controller, controller_alternative1, controller_alternative2, tokenHolder, recipient, unknown]) {
    ...
    describe('when certificate is valid', function () {
      describe('when the operator is authorized', function () {
        describe('when balance is sufficient', function () {
              ...
              describe('when receiver is eligible', function () {
                describe('when the amount is a multiple of the granularity', function () {
                  it('returns Ethereum status code A2 (canTransferByPartition)', async function () {
                    let cert = await getValidCertificate(this.token, 'transferByPartition', tokenHolder);
                    const response = await this.token.canTransferByPartition(
                      partition1, recipient, amount, cert, { from: tokenHolder });
                    await assertEscResponse(response, ESC_A2, EMPTY_BYTE32, partition1);
                  });
                  // @leonod method commented on contract to save opcodes
                  /* it('returns Ethereum status code A2 (canOperatorTransferByPartition)', async function () {
                    let cert = await getValidCertificate(this.token, 'canOperatorTransferByPartition', tokenHolder);
                    const response = await this.token.canOperatorTransferByPartition(
                      partition1, tokenHolder, recipient, amount, ZERO_BYTE, cert, { from: tokenHolder });
                    await assertEscResponse(response, ESC_A2, EMPTY_BYTE32, partition1);
                  }); */
                });
                describe('when the amount is not a multiple of the granularity', function () {
                  it('returns Ethereum status code A9', async function () {
                    let cert = await getValidCertificate(this.token, 'transferByPartition', tokenHolder);
                    const response = await this.token.canTransferByPartition(
                      partition1, recipient, 1, cert, { from: tokenHolder });
                    await assertEscResponse(response, ESC_A9, EMPTY_BYTE32, partition1);
                  });
                });
              });
              // @leonod Updated because the behavior is not the same anymore : the certificatecontroller works
              describe('when receiver is not eligible', async function () {
          ...
          describe('when receiver is the zero address', function () {
            it('returns Ethereum status code A6', async function () {
              let cert = await getValidCertificate(this.token, 'transferByPartition', tokenHolder);
              const response = await this.token.canTransferByPartition(
                partition1, ZERO_ADDRESS, amount, cert, { from: tokenHolder });
        ...
        describe('when balance is not sufficient', function () {
          it('returns Ethereum status code A4 (insuficient global balance)', async function () {
            let cert = await getValidCertificate(this.token, 'transferByPartition', tokenHolder);
            shouldFail(this.token.transferByPartition(
              partition1, recipient, issuanceAmount + localGranularity, cert, { from: tokenHolder }));
          });
          it('returns Ethereum status code A4 (insuficient partition balance)', async function () {
            let cert = await getValidCertificate(this.token, 'issueByPartition', owner);
            await this.token.issueByPartition(
              partition2, tokenHolder, localGranularity, cert, { from: owner });
            cert = await getValidCertificate(this.token, 'transferByPartition', tokenHolder);
            const response = await this.token.canTransferByPartition(
              partition2, recipient, amount, cert, { from: tokenHolder });
      ...
      /* describe('when the operator is not authorized', function () {
       it('returns Ethereum status code A7 (canOperatorTransferByPartition)', async function () {
          let cert = await getValidCertificate(this.token, 'canOperatorTransferByPartition', tokenHolder);
          const response = await this.token.canOperatorTransferByPartition(
            partition1, operator, recipient, amount, ZERO_BYTE, cert, { from: tokenHolder });
    ...
    describe('when sender is the issuer', function () {
      describe('when token is issuable', function () {
        ...
        it('emits a issuedByPartition event', async function () {
          let cert = await getValidCertificate(this.token, 'issueByPartition', owner);
          const { logs } = await this.token.issueByPartition(partition1, tokenHolder, issuanceAmount, cert, { from: owner });
          ...
          assert.equal(logs[2].args.data, cert);
      ...
      describe('when token is not issuable', function () {
        ...
        /* it('reverts', async function () {
          ...
          assert.isTrue(!(await this.token.isIssuable()));
          let cert = await getValidCertificate(this.token, 'issueByPartition', owner);
          await shouldFail.reverting(this.token.issueByPartition(partition1, tokenHolder, issuanceAmount, cert, { from: owner }));
        }); */
      });
    });
    describe('when sender is not the issuer', function () {
      it('reverts', async function () {
        let cert = await getValidCertificate(this.token, 'issueByPartition', unknown);
        await shouldFail.reverting(this.token.issueByPartition(partition1, tokenHolder, issuanceAmount, cert, { from: unknown }));
      });
    });
  });
  ...
  describe('redeemByPartition', function () {
    ...
    describe('when the redeemer has enough balance for this partition', function () {
      it('redeems the requested amount', async function () {
        let cert = await getValidCertificate(this.token, 'redeemByPartition', tokenHolder);
        await this.token.redeemByPartition(partition1, redeemAmount, cert, { from: tokenHolder });
      ...
      it('emits a redeemedByPartition event', async function () {
        let cert = await getValidCertificate(this.token, 'redeemByPartition', tokenHolder);
        const { logs } = await this.token.redeemByPartition(partition1, redeemAmount, cert, { from: tokenHolder });

        assert.equal(logs.length, 3);

        assertBurnEvent(logs, partition1, tokenHolder, tokenHolder, redeemAmount, cert, null);
      });
    });
    describe('when the redeemer does not have enough balance for this partition', function () {
      it('reverts', async function () {
        let cert = await getValidCertificate(this.token, 'redeemByPartition', tokenHolder);
        await shouldFail.reverting(this.token.redeemByPartition(partition2, redeemAmount, cert, { from: tokenHolder }));
      });
    });
    describe('special case (_removeTokenFromPartition shall revert)', function () {
      it('reverts', async function () {
        let cert = await getValidCertificate(this.token, 'issueByPartition', owner);
        await this.token.issueByPartition(partition2, owner, issuanceAmount, cert, { from: owner });
        cert = await getValidCertificate(this.token, 'issueByPartition', tokenHolder);
        await shouldFail.reverting(this.token.redeemByPartition(partition2, 0, cert, { from: tokenHolder }));
      });
    });
  });
  ...
  describe('operatorRedeemByPartition', function () {
    ...
    describe('when the sender is an operator for this partition', function () {
      describe('when the redeemer has enough balance for this partition', function () {
        it('redeems the requested amount', async function () {
          await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
          let cert = await getValidCertificate(this.token, 'operatorRedeemByPartition', operator);
          await this.token.operatorRedeemByPartition(partition1, tokenHolder, redeemAmount, ZERO_BYTE, cert, { from: operator });
        ...
        it('emits a redeemedByPartition event', async function () {
          await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
          let cert = await getValidCertificate(this.token, 'operatorRedeemByPartition', operator);
          const { logs } = await this.token.operatorRedeemByPartition(partition1, tokenHolder, redeemAmount, ZERO_BYTE, cert, { from: operator });

          assert.equal(logs.length, 3);

          assertBurnEvent(logs, partition1, operator, tokenHolder, redeemAmount, null, cert);
        });
      });
      describe('when the redeemer does not have enough balance for this partition', function () {
        it('reverts', async function () {
          it('redeems the requested amount', async function () {
            await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
            let cert = await getValidCertificate(this.token, 'operatorRedeemByPartition', operator);
            await shouldFail.reverting(this.token.operatorRedeemByPartition(partition1, tokenHolder, issuanceAmount + 1, ZERO_BYTE, cert, { from: operator }));
          });
        });
      });
    });
    ...
    describe('when the sender is not an operator', function () {
      it('reverts', async function () {
        let cert = await getValidCertificate(this.token, 'operatorRedeemByPartition', owner);
        await shouldFail.reverting(this.token.operatorRedeemByPartition(partition1, tokenHolder, redeemAmount, ZERO_BYTE, cert, { from: operator }));
      });
    });
  });
  ...
  describe('transferByPartition', function () {
    ...
    describe('when the sender has enough balance for this partition', function () {
      describe('when the transfer amount is not equal to 0', function () {
        it('transfers the requested amount', async function () {
          ...
          let cert = await getValidCertificate(this.token, 'transferByPartition', tokenHolder);
          await this.token.transferByPartition(partition1, recipient, transferAmount, cert, { from: tokenHolder });
          cert = await getValidCertificate(this.token, 'transferByPartition', tokenHolder);
          await this.token.transferByPartition(partition1, recipient, 0, cert, { from: tokenHolder });
        ...
        it('emits a TransferByPartition event', async function () {
          let cert = await getValidCertificate(this.token, 'transferByPartition', tokenHolder);
          const { logs } = await this.token.transferByPartition(partition1, recipient, transferAmount, cert, { from: tokenHolder });

          assert.equal(logs.length, 3);

          assertTransferEvent(logs, partition1, tokenHolder, tokenHolder, recipient, transferAmount, cert, null);
        });
      });
      describe('when the transfer amount is equal to 0', function () {
        it('reverts', async function () {
          let cert = await getValidCertificate(this.token, 'transferByPartition', tokenHolder);
          await shouldFail.reverting(this.token.transferByPartition(partition2, recipient, 0, cert, { from: tokenHolder }));
        });
      });
    ...
    describe('when the sender does not have enough balance for this partition', function () {
      it('reverts', async function () {
        let cert = await getValidCertificate(this.token, 'transferByPartition', tokenHolder);
        await shouldFail.reverting(this.token.transferByPartition(partition2, recipient, transferAmount, cert, { from: tokenHolder }));
      });
    });
  });
  ...
  describe('operatorTransferByPartition', function () {
    ...
    describe('when the sender is an operator for this partition', function () {
      describe('when the sender has enough balance for this partition', function () {
        describe('when partition does not change', function () {
          it('transfers the requested amount', async function () {
            ...
            await this.token.operatorTransferByPartition(partition1, tokenHolder, recipient, transferAmount, ZERO_BYTE, cert, { from: operator });
          ...
          it('transfers the requested amount with attached data (without changePartition flag)', async function () {
            ...
            await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
            let cert = await getValidCertificate(this.token, 'operatorTransferByPartition', operator);
            await this.token.operatorTransferByPartition(partition1, tokenHolder, recipient, transferAmount, doNotChangePartition, cert, { from: operator });
          ...
          it('emits a TransferByPartition event', async function () {
            await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
            let cert = await getValidCertificate(this.token, 'operatorTransferByPartition', operator);
            const { logs } = await this.token.operatorTransferByPartition(partition1, tokenHolder, recipient, transferAmount, ZERO_BYTE, cert, { from: operator });

            assert.equal(logs.length, 3);

            assertTransferEvent(logs, partition1, operator, tokenHolder, recipient, transferAmount, null, cert);
          });
        });
        describe('when partition changes', function () {
          ...
          it('emits a changedPartition event', async function () {
            await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
            let cert = await getValidCertificate(this.token, 'operatorTransferByPartition', operator);
            const { logs } = await this.token.operatorTransferByPartition(partition1, tokenHolder, recipient, transferAmount, changeToPartition2, cert, { from: operator });

            assert.equal(logs.length, 4);

            assertTransferEvent([logs[0], logs[1], logs[2]], partition1, operator, tokenHolder, recipient, transferAmount, changeToPartition2, cert);
      ...
      describe('when the sender does not have enough balance for this partition', function () {
        it('reverts', async function () {
          await this.token.authorizeOperatorByPartition(partition1, operator, { from: tokenHolder });
          let cert = await getValidCertificate(this.token, 'operatorTransferByPartition', operator);
          await shouldFail.reverting(this.token.operatorTransferByPartition(partition1, tokenHolder, recipient, issuanceAmount + 1, ZERO_BYTE, cert, { from: operator }));
        });
      });
    });
    describe('when the sender is a global operator', function () {
    ...
    describe('when the sender is not an operator', function () {
      it('reverts', async function () {
        let cert = await getValidCertificate(this.token, 'operatorTransferByPartition', operator);
        await shouldFail.reverting(this.token.operatorTransferByPartition(partition1, tokenHolder, recipient, transferAmount, ZERO_BYTE, cert, { from: operator }));
      });
    });
  });
  ...
  describe('totalPartitions', function () {
    ...
    describe('when tokens are issued for 3 partitions', function () {
      it('returns list of 3 partitions', async function () {
        ...
        await this.token.issueByPartition(partition1, tokenHolder, issuanceAmount, cert, { from: owner });
        cert = await getValidCertificate(this.token, 'issueByPartition', owner);
        await this.token.issueByPartition(partition2, recipient, issuanceAmount, cert, { from: owner });
        cert = await getValidCertificate(this.token, 'issueByPartition', owner);
        await this.token.issueByPartition(partition3, unknown, issuanceAmount, cert, { from: owner });
        const partitionsOf = await this.token.totalPartitions();
        assert.equal(partitionsOf.length, 3);
        assert.equal(partitionsOf[0], partition1);
        assert.equal(partitionsOf[1], partition2);
  ...
  describe('transferWithData', function () {
    describe('when defaultPartitions have been defined', function () {
      ...
      describe('when the sender has enough balance for those default partitions', function () {
        describe('when the sender has defined custom default partitions', function () {
          it('transfers the requested amount', async function () {
            ...
            await this.token.transferWithData(recipient, 2.5 * issuanceAmount, cert, { from: tokenHolder });
          ...
          it('emits a sent event', async function () {
            await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
            let cert = await getValidCertificate(this.token, 'transferWithData', tokenHolder);
            const { logs } = await this.token.transferWithData(recipient, 2.5 * issuanceAmount, cert, { from: tokenHolder });

            assert.equal(logs.length, 1 + 2 * partitions.length);

            assertTransferEvent([logs[0], logs[1], logs[2]], partition3, tokenHolder, tokenHolder, recipient, issuanceAmount, cert, null);
            assertTransferEvent([logs[3], logs[4]], partition1, tokenHolder, tokenHolder, recipient, issuanceAmount, cert, null);
            assertTransferEvent([logs[5], logs[6]], partition2, tokenHolder, tokenHolder, recipient, 0.5 * issuanceAmount, cert, null);
          });
        });
      ...
      describe('when the sender does not have enough balance for those default partitions', function () {
        it('reverts', async function () {
          await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
          let cert = await getValidCertificate(this.token, 'transferWithData', tokenHolder);
          await shouldFail.reverting(this.token.transferWithData(recipient, 3.5 * issuanceAmount, cert, { from: tokenHolder }));
        });
      });
    });
    describe('when defaultPartitions have not been defined', function () {
      it('reverts', async function () {
        ...
        await issueOnMultiplePartitions(this.token, owner, tokenHolder, partitions, [issuanceAmount, issuanceAmount, issuanceAmount]);
        let cert = await getValidCertificate(this.token, 'transferWithData', tokenHolder);
        await shouldFail.reverting(this.token.transferWithData(recipient, 2.5 * issuanceAmount, cert, { from: tokenHolder }));
      });
    });
  });
  ...
  describe('_transferByDefaultPartitions', function () {
    ...
    describe('when the sender has enough balance for those default partitions', function () {
      it('transfers the requested amount (scenario1)', async function () {
        let cert = await getValidCertificate(this.token, 'issueByPartition', owner);
        await this.token.issueByPartition(partitions[0], tokenHolder, 2*issuanceAmount, cert, { from: owner });
        cert = await getValidCertificate(this.token, 'issueByPartition', owner);
        await this.token.issueByPartition(partitions[2], tokenHolder, 2*issuanceAmount, cert, { from: owner });
      ...
      it('transfers the requested amount (scenario2)', async function () {
        ...
        await this.token.issueByPartition(partitions[0], tokenHolder, 2*issuanceAmount, cert, { from: owner });
        cert = await getValidCertificate(this.token, 'issueByPartition', owner);
        await this.token.issueByPartition(partitions[1], tokenHolder, 2*issuanceAmount, cert, { from: owner });
        await this.token.setDefaultPartitions(partitions, { from: owner });
      ...
      describe('when defaultPartitions have been defined', function () {
        describe('when the sender has enough balance for those default partitions', function () {
          it('transfers the requested amount', async function () {
            ...
            await this.token.transferFromWithData(tokenHolder, recipient, 2.5 * issuanceAmount, ZERO_BYTE, cert, { from: operator });
          ...
          it('emits a sent event', async function () {
            await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
            let cert = await getValidCertificate(this.token, 'transferFromWithData', operator);
            const { logs } = await this.token.transferFromWithData(tokenHolder, recipient, 2.5 * issuanceAmount, ZERO_BYTE, cert, { from: operator });

            assert.equal(logs.length, 1 + 2 * partitions.length);

            assertTransferEvent([logs[0], logs[1], logs[2]], partition3, operator, tokenHolder, recipient, issuanceAmount, null, cert);
            assertTransferEvent([logs[3], logs[4]], partition1, operator, tokenHolder, recipient, issuanceAmount, null, cert);
            assertTransferEvent([logs[5], logs[6]], partition2, operator, tokenHolder, recipient, 0.5 * issuanceAmount, null, cert);
          });
        });
        describe('when the sender does not have enough balance for those default partitions', function () {
          it('reverts', async function () {
            await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
            let cert = await getValidCertificate(this.token, 'transferFromWithData', operator);
            await shouldFail.reverting(this.token.transferFromWithData(tokenHolder, recipient, 3.5 * issuanceAmount, ZERO_BYTE, cert, { from: operator }));
          });
        });
      });
      describe('when defaultPartitions have not been defined', function () {
        it('reverts', async function () {
          ...
          let cert = await getValidCertificate(this.token, 'transferFromWithData', operator);
          await shouldFail.reverting(this.token.transferFromWithData(tokenHolder, recipient, 2.5 * issuanceAmount, ZERO_BYTE, cert, { from: operator }));
        });
      });
    });
    ...
    describe('when defaultPartitions have been defined', function () {
      describe('when the sender has enough balance for those default partitions', function () {
        it('redeeems the requested amount', async function () {
          ...
          await this.token.redeem(2.5 * issuanceAmount, cert, { from: tokenHolder });
        ...
        it('emits a redeemedByPartition events', async function () {
          await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
          let cert = await getValidCertificate(this.token, 'redeem', tokenHolder);
          const { logs } = await this.token.redeem(2.5 * issuanceAmount, cert, { from: tokenHolder });

          assert.equal(logs.length, 1 + 2 * partitions.length);

          assertBurnEvent([logs[0], logs[1], logs[2]], partition3, tokenHolder, tokenHolder, issuanceAmount, cert, null);
          assertBurnEvent([logs[3], logs[4]], partition1, tokenHolder, tokenHolder, issuanceAmount, cert, null);
          assertBurnEvent([logs[5], logs[6]], partition2, tokenHolder, tokenHolder, 0.5 * issuanceAmount, cert, null);
        });
      });
      describe('when the sender does not have enough balance for those default partitions', function () {
        it('reverts', async function () {
          await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
          let cert = await getValidCertificate(this.token, 'redeem', tokenHolder);
          await shouldFail.reverting(this.token.redeem(3.5 * issuanceAmount, cert, { from: tokenHolder }));
        });
      });
    });
    describe('when defaultPartitions have not been defined', function () {
      it('reverts', async function () {
        await this.token.setDefaultPartitions([], { from: owner });
        let cert = await getValidCertificate(this.token, 'redeem', tokenHolder);
        await shouldFail.reverting(this.token.redeem(2.5 * issuanceAmount, cert, { from: tokenHolder }));
      });
    });
  });
      ...
      describe('when defaultPartitions have been defined', function () {
        describe('when the sender has enough balance for those default partitions', function () {
          it('redeems the requested amount', async function () {
            ...
            await this.token.redeemFrom(tokenHolder, 2.5 * issuanceAmount, ZERO_BYTE, cert, { from: operator });

            await assertBalances(this.token, tokenHolder, partitions, [0, 0.5 * issuanceAmount, 0]);
          });
          it('emits redeemedByPartition events', async function () {
            await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
            let cert = await getValidCertificate(this.token, 'redeemFrom', tokenHolder);
            const { logs } = await this.token.redeemFrom(tokenHolder, 2.5 * issuanceAmount, ZERO_BYTE, cert, { from: operator });

            assert.equal(logs.length, 1 + 2 * partitions.length);

            assertBurnEvent([logs[0], logs[1], logs[2]], partition3, operator, tokenHolder, issuanceAmount, null, cert);
            assertBurnEvent([logs[3], logs[4]], partition1, operator, tokenHolder, issuanceAmount, null, cert);
            assertBurnEvent([logs[5], logs[6]], partition2, operator, tokenHolder, 0.5 * issuanceAmount, null, cert);
          });
        });
        describe('when the sender does not have enough balance for those default partitions', function () {
          it('reverts', async function () {
            await this.token.setDefaultPartitions(reversedPartitions, { from: owner });
            let cert = await getValidCertificate(this.token, 'redeemFrom', tokenHolder);
            await shouldFail.reverting(this.token.redeemFrom(tokenHolder, 3.5 * issuanceAmount, ZERO_BYTE, cert, { from: operator }));
          });
        });
      });
      describe('when defaultPartitions have not been defined', function () {
        it('reverts', async function () {
          await this.token.setDefaultPartitions([], { from: owner });
          let cert = await getValidCertificate(this.token, 'redeemFrom', tokenHolder);
          await shouldFail.reverting(this.token.redeemFrom(tokenHolder, 2.5 * issuanceAmount, ZERO_BYTE, cert, { from: operator }));
        });
      });
    });
    describe('when the operator is not approved', function () {
...
contract('ERC1400Partition', function ([owner, operator, controller, controller_alternative1, controller_alternative2, tokenHolder, recipient, unknown]) {
  ...
  describe('ERC1400Partition - redeem', function () {
    ...
    //   let cert = await getValidCertificate(this.token, 'redeem', tokenHolder);
    //   await this.token.redeem(500, cert, { from: tokenHolder });
    //   await assertBalance(this.token, tokenHolder, 1000);
    // });
    it('reverts', async function () {
      await assertBalance(this.token, tokenHolder, 1000);
      let cert = await getValidCertificate(this.token, 'redeem', tokenHolder);
      await shouldFail.reverting(this.token.redeem(500, cert, { from: tokenHolder }));
    });
  });
  ...
  describe('ERC1400Partition - redeemFrom', function () {
    beforeEach(async function () {
      this.token = await ERC1400Partition.new('ERC1410Token', 'DAU', 1, [controller], CERTIFICATE_SIGNER, partitions, tokenHolder, 1000);
    });
    // it('redeemFrom function is disactivated', async function () {
    //   await this.token.authorizeOperator(operator, { from: tokenHolder });
    //   await assertBalance(this.token, tokenHolder, 1000);
    //   let cert = await getValidCertificate(this.token, 'redeem', tokenHolder);
    //   await this.token.redeemFrom(tokenHolder, 500, ZERO_BYTE, cert, { from: operator });
    //   await assertBalance(this.token, tokenHolder, 1000);
    // });
    it('reverts', async function () {
      await this.token.authorizeOperator(operator, { from: tokenHolder });
      await assertBalance(this.token, tokenHolder, 1000);
      let cert = await getValidCertificate(this.token, 'redeem', tokenHolder);
      await shouldFail.reverting(this.token.redeemFrom(tokenHolder, 500, ZERO_BYTE, cert, { from: operator }));
    });
  });
});
